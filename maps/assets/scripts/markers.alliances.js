async function MarkersAlliances(world) {
    ...
    for (const one in datas){
        ...
        if (data.alliance.id == 0) {
            ...
            popup = '<a href="/rp/civilisation/'+data.alliance.id+'" class="button is-TD-smoothwhite" style="height: 30px;">'+data.alliance.name+'</a>';
        }
        tooltip = '';
        icon = 'udbIcon';
        for (let polygon in data.polygons.villes) {
            let subdata = data.polygons.villes[polygon];
            polygons.push({type: subdata.shape, dbid: subdata.cartoid, option: "civ", authorisation: data.authorisation, coords: subdata.coords, color: data.alliance.color, text: subdata.text, icon: subdata.inactif ? yellowIcon : icon, popup: popup, tooltip: tooltip});
        }
        for (let polygon in data.polygons.quartiers) {
            let subdata = data.polygons.quartiers[polygon];
            polygons.push({type: subdata.shape, dbid: subdata.cartoid, option: "quartier", authorisation: data.authorisation, coords: subdata.coords, color: data.alliance.color, text: subdata.text, icon: subdata.inactif ? yellowIcon : icon, popup: popup, tooltip: tooltip});
        }
